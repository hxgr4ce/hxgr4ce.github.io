/* Helper functions for use with three.js.
...
TW.makeColor = function (r, g, b, debug) {
  ...
  var intValue =
    256 * 256 * unsignedByte(r) + 256 * unsignedByte(g) + unsignedByte(b);
  if (debug || TW.debug) {
...
TW.setKeyboardCallback = function (charstring, callback, doc, shared) {
  ...
  if (TW.debug) {
    console.log(
      "set kbd: " + charstring + "," + (shared ? "shared" : "not-shared")
    );
  }
  if (shared) {
    var obj = TW;
  ...
  obj.keyboard_callbacks[charstring] =
    doc + (shared ? " shared" : " not-shared");
};

TW.showKeyboardCallbacks = function () {
  var state = TW.lastClickTarget.TW_state;
  var i,
    list = state.keyboard_callbacks_docs,
    len = list.length;
...
TW.render = function () {
  ...
  if (state.render) {
    ...
    console.log(
      "No render function in the state object; \n" +
...
TW.toggleBoundingBox = function (mode) {
  ...
  if (mode === "show") {
    ...
    // this is the normal usage
    state.sceneBoundingBoxHelper.visible =
      !state.sceneBoundingBoxHelper.visible;
  }
  state.render();
};
...
TW.degrees2radians = function (degrees) {
  return (Math.PI / 180.0) * degrees;
};

TW.radians2degrees = function (radians) {
...
TW.setMaterialForFaces = function (geom, materialIndex, faceIndexes) {
  ...
  for (; i < len; ++i) {
    ...
    if (TW.debug) {
      console.log(
        "setting face " + faceIndex + " to material " + materialIndex
      );
    }
...
TW.addGroundHelper = function (scene, bb) {
  ...
  var groundGeom = new THREE.PlaneGeometry(
    bb.maxx - bb.minx,
    bb.maxz - bb.minz
  );
  var groundMat = new THREE.MeshBasicMaterial({
    color: TW.groundColor,
...
TW.viewFromFront = function () {
  ...
  state.cameraObject.position.set(
    center.x,
    center.y,
    center.z + cam.cameraRadius
  );
...
TW.mainInit = function (renderer, scene, inits) {
  ...
  if (
    !(
      renderer &&
      renderer.domElement &&
      renderer.domElement.nodeName === "CANVAS"
    )
  ) {
...
TW.cameraSetupParams = function (sceneBoundingBox, fovy) {
  ...
  if (!fovy || isNaN(fovy)) throw "fovy is missing or invalid: " + fovy;
  var sbb = sceneBoundingBox;
  var xSize = sbb.maxx - sbb.minx;
  var ySize = sbb.maxy - sbb.miny;
  var zSize = sbb.maxz - sbb.minz;
  var diameter = Math.sqrt(xSize * xSize + ySize * ySize + zSize * zSize);
...
TW.cameraSetup = function (renderer, scene, scene_bb, inits) {
  ...
  if (!bb) {
    console.log(
      "without the scene bounding box, I have no idea where to position the camera.\n" +
  ...
  if (TW.lastClickTarget === null) {
    console.warn(
      "It seems that you did not invoke TW.mainInit; doing it for you."
    );
  ...
  if (ah == null) {
    throw (
      "Your scene has no AxisHelper, which means it wasn't created by " +
      "TW.mainInit()."
    );
  }
...
TW.createTeddyBear = function (params) {
  ...
  function createLimb(radiusTop, radiusBottom, length, params) {
    ...
    var limbGeom = new THREE.CylinderGeometry(
      radiusTop,
      radiusBottom,
      length,
      cd
...
TW.setKeyboardCallback(
  "?",
  TW.showKeyboardCallbacks,
  "show keyboard callbacks",
  true
);
TW.setKeyboardCallback("a", TW.toggleAxes, "toggle axes", true);
TW.setKeyboardCallback(
  "b",
  TW.toggleBoundingBox,
  "toggle scene bounding box",
  true
);
...
TW.setKeyboardCallback(
  ...
  "view obliquely, from above front side (+XYZ)",
  true
);
...
TW.nextPowerOf2 = function (x) {
  ...
  return Math.round(
    Math.exp(Math.log(2) * Math.ceil(Math.log(x) / Math.log(2)))
  );
};
...
TW.isArray2D = function (array2D) {
  ...
  if (len != width * height) {
    throw (
      "array's width and height don't multiply to yield the length: " + array2D
    );
  }
  return true;
};
...
TW.createUSFlagGray = function (log2size) {
  ...
  for (i = 0; i < unionHoist; i++)
    for (j = 0; j < unionFly; j++) TW.rowMajorAset(data, i, j, BLUE);
  return data;
};
...
TW.isArrayRGB = function (arrayRGB) {
  ...
  if (!len) {
    throw (
      "array does not have length defined; is it even an array? " + arrayRGB
    );
  }
  ...
  if (len != width * height * 3) {
    throw (
      "array's width and height and 3 don't multiply to yield the length: " +
      arrayRGB
    );
  }
  return true;
...
TW.createUSFlagRedWhiteBlue = function (log2size) {
  ...
  var unionFly = Math.round(0.76 * hoist); // from flag spec
  for (i = 0; i < unionHoist; i++)
    for (j = 0; j < unionFly; j++) TW.rowMajorAsetRGB(data, i, j, 0, 0, 0xff); // BLUE
  return data;
};

TW.createTriColorFlag = function (
  log2size,
  colorLeft,
  colorMiddle,
  colorRight
...
TW.showFaces = function (geom, domSelector) {
  ...
  for (i = 0; i < numFaces; i++) {
    desc +=
      '<li><span style="color:black;background-color:#' +
      colors[i].getHexString() +
      '">Face ' +
      i +
      " is in this color</span>";
  }
  desc += "</ol>";
...
TW.textureMaterial = function (url) {
  console.log(
    "This function is obsolete; use TW.loadTexture(url,cb) and TW.loadTexturesDone(function);"
  );
  ...
  var texture = new THREE.ImageUtils.loadTexture(
    ...
    function (text) {
      ...
      if (
        !TW.isPowerOf2(text.image.width) ||
      ...
      numTexturesToLoad--;
      if (numTexturesToLoad == 0) TW.render();
    }
  );
...
THREE.BezierSurfaceGeometry.prototype.computeBezierSurfacePoints = function () {
  var sSegments = this.sSegments;
  var tSegments = this.tSegments;
  var cps = this.controlPoints;
  if (TW.debug)
    console.log(
      "computing Bezier surface vertices with: " +
        sSegments +
        " and " +
        tSegments
    );
  ...
  if (TW.debug) console.log("computed " + verts.length + " vertices");
  this.vertices = verts;
  return verts;
  ...
  function bez(s, t) {
    ...
    if (TW.debug) console.log("s: " + s + " t: " + t + " x: " + vert.x);
    return vert;
  }
...
THREE.BezierSurfaceGeometry.prototype.computeBezierSurfaceFaces = function () {
  var sSegments = this.sSegments;
  var tSegments = this.tSegments;
...
TW.perp2 = function (u, v) {
  ...
  var uv = TW.dot(u, v);
  // (uu)v - (uv)u
  return [uu * v[0] - uv * u[0], uu * v[1] - uv * u[1], uu * v[2] - uv * u[2]];
  /*
